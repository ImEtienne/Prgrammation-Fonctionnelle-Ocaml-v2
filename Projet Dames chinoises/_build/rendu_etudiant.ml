...
(*question 11*)
(*let rec remplir (config:configuration) l = 
  match l with
  |[] -> config
  |[e] -> remplir (tourne_config(remplir_triangle config e ((-dim*2),dim,dim))) []
  |e :: l -> remplir (tourne_config (remplir_triangle config e ((-dim*2),dim,dim))) l;;*)
...
(*13*)
(*let rec fait_dep_unit_aux case_col_list c1 c2 coll =
  match case_col_list with
    []-> (c2,coll)::[]
  | (c,col):: q -> if c = c1 then  fait_dep_unit_aux q c1 c2 col
    else (c,col):: fait_dep_unit_aux q c1 c2 col ;;

  let fait_dep_unit config c1 c2 =
  let (case_col_list,col_list)= config in
  match col_list with
    []-> config
  | t::q -> (((fait_dep_unit_aux case_col_list c1 c2 t),col_list):configuration);;*)
